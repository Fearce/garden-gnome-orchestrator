/**
 * UI/store regression gate for schedule controls. A schedule write must project immediately and request
 * an authoritative resync instead of waiting for the 20-second heartbeat; a disconnected write must fail
 * visibly so the editor can stay open.
 */
...
assert.deepEqual(socket.sent.map((frame) => frame.type), ["schedule.create", "snapshot.request"]);
...
assert.deepEqual(socket.sent, [
  { type: "schedule.update", id: "schedule-1", patch: { enabled: false } },
...
assert.deepEqual(socket.sent.map((frame) => frame.type), ["schedule.delete", "snapshot.request"]);
...
console.log("Scheduled-task controls update and resync immediately, and disconnected writes stay visible.");
process.exit(0);
