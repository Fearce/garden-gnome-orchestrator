...
/** Opens the Git console — the repo-level fetch/pull/push/branch/commit surface that replaces reaching
 *  for GitHub Desktop. The mark is GitHub's because that is the muscle memory it stands in for; the
 *  console itself works against any git remote. */
function GitButton({ open, onToggle }: { open: boolean; onToggle: () => void }) {
  return (
    <button
      className={"git-btn" + (open ? " on" : "")}
      title="Git — branches, fetch, pull, push, commit"
      aria-label="Open Git"
      aria-expanded={open}
      onClick={onToggle}
    >
