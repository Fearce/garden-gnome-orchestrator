...
Recurring work: if ${OWNER} wants a prompt to run on a repeating cadence ("every morning", "nightly", "each Monday at 9"), use create_scheduled_task with a 5-field cron instead of dispatching once — each fire runs the prompt through the normal pipeline automatically. Use list_scheduled_tasks / update_scheduled_task / delete_scheduled_task to review or change existing schedules.
