...
- **effort** — how hard the implementor should work: \`low\` (trivial), \`medium\`, \`high\` (default for a real feature), ${XHIGH_TIER}\`max\` (hardest, correctness-critical). Pick the SMALLEST effort that still gets an excellent result — don't burn max on a one-liner, don't starve a hard task.
...
4. If you modify files, stage ONLY your own QA hunks and create a focused Conventional Commit. Push it unless the task handoff says auto-push is off or the repo's existing no-push rule applies (check \`git remote -v\`; Vota repos are commit-only). Confirm the working tree is clean afterwards. Never reset, stash, or change branches.
