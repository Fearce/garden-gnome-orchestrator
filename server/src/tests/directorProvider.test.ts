...
try {
  ...
  const originalPriority = internals.temporaryAccountPriority;
  const originalTargets = internals.directorTargets;
  const originalReady = internals.directorTargetReady;
  const originalPreferred = internals.preferredDirectorTarget;
  const personalTarget = { key: "claude|acct1|opus", provider: "claude", accountId: "acct1", accountLabel: "personal", model: "opus", capacity: "ready" };
  const stickyCodexTarget = { key: "codex|openai-codex|gpt", provider: "codex", accountId: "openai-codex", accountLabel: "Codex", model: "gpt", capacity: "ready" };
  internals.temporaryAccountPriority = () => ({ accountId: "acct1", until: Date.now() + 60_000 });
  internals.directorTargets = () => [personalTarget, stickyCodexTarget];
  internals.directorTargetReady = () => true;
  internals.preferredDirectorTarget = (targets: Array<{ provider: string }>) => targets.find((target) => target.provider === "codex") ?? targets[0];
  directorInternals.target = stickyCodexTarget;
  check("a temporary owner priority moves a sticky Director to personal", (await directorInternals.chooseTarget())?.key === personalTarget.key);
  internals.temporaryAccountPriority = () => null;
  check("without priority the Director keeps its ready sticky target", (await directorInternals.chooseTarget())?.key === stickyCodexTarget.key);
  db.kvSet("director_temporary_priority_until", String(Date.now() - 1));
  directorInternals.target = personalTarget;
  check("an expired priority releases the sticky personal target", (await directorInternals.chooseTarget())?.key === stickyCodexTarget.key);
  check("the expired Director priority marker is cleared", db.kvGet("director_temporary_priority_until") === null);
  internals.temporaryAccountPriority = originalPriority;
  internals.directorTargets = originalTargets;
  internals.directorTargetReady = originalReady;
  internals.preferredDirectorTarget = originalPreferred;
  directorInternals.target = configured[0];
