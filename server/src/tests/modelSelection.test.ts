...
Work on the active branch, commit using Conventional Commits, and push when complete unless the origin matches the configured commit-only rule. Because this changes server code on Windows, deploy via the supported \`npm run deploy --prefix server\` atomic deployment path—not stop/start or a hand-built shared artifact—then verify the live build with the supported verification path after resume. Leave a concise evidence-backed handoff covering root cause, fix, tests, deployment/live verification, and any remaining risk. No placeholders or half-measures.`,
...
Use the active branch, inspect established decisions before architectural changes, run focused and proportionate broader tests, commit with a Conventional Commit, and push unless the origin matches the configured commit-only rule. Because server code changes are expected, deploy via \`npm run deploy --prefix server\` only; never stop/start or hand-build the shared artifact. After automatic resume, run the supported live verification and leave evidence of root causes, regressions, deployment, live-build verification, and active Android-task continuity. No placeholders or partial fixes.`,
} as const;
