...
/**
 * Exposes Kevin's global memory to the director and researcher: a lexical
...
export function createMemoryServer(memory: MemoryService): McpServerConfig {
  const searchMemory = tool(
    "search_memory",
    "Search Kevin's global memory (his stack, preferences, prior decisions, lessons learned, project state) for context relevant to a query. Returns the most relevant memory files with their one-line descriptions and paths. Read a returned path for full detail. ALWAYS check this before dispatching work — it surfaces context Kevin assumes you already know.",
    {
      query: z.string().describe("What to look for, e.g. 'trading bot supervision rules' or 'design taste preferences'."),
