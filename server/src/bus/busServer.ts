...
export function createBusServer(api: OrchestratorApi, ctx: BusContext): McpServerConfig {
  ...
  const askUser = tool(
    "ask_user",
    "Ask Kevin for help when you hit a blocker only HE can resolve — a missing file/credential, a needed secret or access, or a decision you can't make yourself. Pauses this task until he answers. Use it EARLY: the moment you identify a hard blocker, ask — do NOT spend turns hunting workarounds for something he can fix in seconds. Prefer multiple-choice options when you can.",
    {
      header: z.string().describe("A 1-3 word chip label, e.g. 'Missing creds'."),
      question: z.string().describe("What you need from Kevin, with enough context for him to act."),
      options: z
