...
| Put recurring-free inference APIs directly into **task failover**? | **Not until they have a real coding-agent harness.** Nine providers now have a secure connection lab, dynamic/free-safe model checks, normalized streaming/tools/errors, mandatory usage chips, and explicit probes. None can impersonate Claude/Codex/Grok/z.ai's workspace, resume, steering, bus, or commit contract, so automatic paid/fake failover stays impossible. | [`free-ai-provider-connections.md`](free-ai-provider-connections.md); `npm run test:free-providers` |
