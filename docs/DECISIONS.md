...
| Use **LiveBench scores plus cost/token burn** when auto-selecting models and effort? | **Yes.** LiveBench is a daily cached secondary prior; durable local grades are stronger and retain quality, QA, whole-pipeline dollars/turns/time, normalized token categories, and model×effort outcomes beyond task purging. `$0` subscription cost never hides scarce allowance burn. | [`livebench-model-selection.md`](livebench-model-selection.md) · `npm run test:livebench` · `npm run test:model-select` · `npm run test:auto-model` |
| Put recurring-free inference APIs directly into **task failover**? | **Yes for bounded read-only roles; no for coding roles.** Nine providers have secure connections and mandatory usage chips. OpenRouter is restricted to its live exact-zero `:free` catalog routes and documented Free Models Router, with provider fallbacks disabled. A provider-neutral, path-confined tool loop now offloads planner and read-lane tasks only when the exact live free model reports tools; any failure falls back to the unchanged paid-provider ladder. Implementor/researcher/QA remain excluded until their write, web, shell, resume, steering, bus, and commit contracts exist. | [`free-ai-provider-connections.md`](free-ai-provider-connections.md); `npm run test:free-providers` |
