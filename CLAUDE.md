...
**The Codex roster has a version FLOOR, and the owner-facing reason is not the model's to free-form.** Two deterministic guards in `modelSelector.ts`, both from the owner's standing "never use versions lower than gpt-5.6 — Luna/Terra/Sol at different efforts, and gpt-5.5 on extra-high makes no sense" directive. (1) `filterAutoSelectionCandidates` drops every older Codex id (`gpt-5.5`, `gpt-5.4*`, `gpt-4.1`, `o3`, `codex-*`) from automatic selection **whenever any GPT-5.6+ Codex model is dispatchable** — Luna at a low effort is meant to beat those on both quality and cost. It is a filter, never a block: when legacy is the ONLY Codex catalog it stays selectable, with `autoSelectableEffortsForCandidate` capping it at `high` so no spend goes past that on an outdated tier. The same floor moved `isPolicyApprovedFlagship`'s Codex arm to Astra + GPT-5.6+ (`gpt-5.6-sol`); Terra/Luna stay workhorses. (2) A **frontier-tier pick may not be announced as avoiding frontier spend** — the reported defect was Astra, the most expensive model on the roster, published as "without needing frontier spend". The prompt instructs the selector to justify the spend, and `scrubFrontierAvoidance` is the deterministic half that survives one bad JSON string. It works at CLAUSE level on purpose: splicing a replacement phrase in place of the matched words shipped broken English into the same owner-facing finding ("picked to using frontier-tier capacity deliberately while keeping quality"), so the claim's clause is dropped whole, any complete clause before it is kept, and one honest sentence is appended. A reason that argues FOR the spend is left byte-for-byte — that is the evidence the owner reads. Gate: `test:model-select`.
