...
The web app at `:4317`/`:4319` is password-gated. **The password is `31593009`.** Don't go
...
await page.request.post("http://127.0.0.1:4317/api/login", { data: { password: "31593009" } });
await page.goto("http://127.0.0.1:4317/");   // now past the login gate
```
```bash
# curl: save the cookie jar, then hit authed routes with it.
curl -s -c /tmp/cj.txt -X POST http://127.0.0.1:4317/api/login -H 'content-type: application/json' -d '{"password":"31593009"}'
