...
**"Pre-existing, not my diff" is where both earlier sessions stopped, and it is not a stopping point.** `bash ~/Claude/tools/gate-blame.sh <npm-script>` answers whose diff it is safely, and for CI `gate-ran.sh` separates ran-and-failed from never-ran. Locally the suite now does that last part itself: a red gate in the summary is tagged `[reported]`, `[crashed]` or `[silent]`. Read the tag before the tail, because for the last two the tail is actively misleading (a stack with no assertion in it, or twelve lines of PASSING output). Gate: `test:gates-driver`.
