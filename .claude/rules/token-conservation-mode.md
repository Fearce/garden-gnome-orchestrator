...
`settings.usageSaving` (per-subscription card, `orchestrator/usageSaving.ts`) is a separate control from the toggle above and is the one that is ON in production. It fires on EITHER meter (5h **or** weekly), and while active it outranks the role override matrix and an auto-selection pick — but **never a strict owner model pin** (implementor-scoped by construction — QA/reviewer/planner still take the saving model unconditionally via `runRole`). A pinned implementor dispatches its exact model and drops the saving policy entirely (`implementorDispatchTarget`), including its effort half: a model+effort pair resolved against ITS model says nothing about another. A pin that wins here still cannot overspend, because `capacitySnapshotForThread` then gates the pinned model's OWN pool, so a short window parks the task visibly instead of substituting. Overriding the pin was the 2026-09-18 "pinning models is broken" report: an Opus-pinned vota task dispatched Sonnet, which that repository does not permit at all. Same 90% default and same `claude-sonnet-5` target as conservation, which is exactly why a report naming one usually means the other — check `setting_usage_saving` before `setting_token_conservation_mode`.
