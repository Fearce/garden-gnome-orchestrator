...
6. **Every** QA recovery allowance is per REVIEW, renewed whenever a round reaches a verdict — a verdict is
   the proof the reviewer is not wedged, which is the only failure these budgets bound. Both of them: the
   turn-ceiling continuation (every round here is an editing pass long enough to hit the ceiling) and the
   empty-run retry. A task-lifetime budget pools unrelated rounds, so a round whose recovery SUCCEEDED
   spends a later round's and parks the task over a mechanism that worked. Keep the lifetime TALLY (`X`,
   what `probe:task-runs` reconciles launches against) and the ALLOWANCE (`XThisRound`, what the loop
   enforces) as separate fields. Gates: `test:qa-budget` N/O + P/Q for behaviour, `test:qa-budget-scope`
   for the shape — it fails if a third budget ever ships enforced against a field nothing renews.
